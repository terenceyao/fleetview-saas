...
| **实时视频** | 设备视频通道选择、实时直播（RTMP/FLV/HLS 三种流）、录制回放、视频文件检索、远程抓拍 |
| **OBD 数据分析** | 车速/转速/油耗/电压/温度多曲线、驾驶行为分析（急加速/急刹/超转）、健康评分与应用建议 |
| **ADAS·DMS 智能筛选** | 告警视频 AI 风险评分（类型权重+新近度+频次）、一键筛选高危片段、设备风险排行、AI 总结结论 |
| 平台资源 | 网关、模型、媒体资源、指令历史 |
| API 控制台 | 可视化调试各端点（含视频直播/回放/抓拍），查看最近请求日志 |
| 设置 | 凭据配置、状态检测、**语言切换（中/英/西）**、数据刷新 |

## 🌍 多语言

内置 **简体中文 / English / Español** 三语运行时翻译引擎（`i18n.js`）：
- 顶栏 `中 / EN / ES` 一键切换，设置持久化到 `localStorage`
- 完整 DOM 遍历翻译 + 数字/百分比模板匹配 + 正则规则兜底
- 覆盖全部 13 个页面（含动态生成的图表提示与错误消息）
...
│       ├── pages3.js       # 页面：实时视频/OBD 分析/ADAS·DMS 智能筛选
│       └── app.js          # 数据聚合、路由、启动引导
└── .gitignore
```
...
视频直播流程：`POST /v3/video/live/start` → 返回 rtmp / flv / hls 三种流地址 → 浏览器端使用 flv.js（FLV）或 hls.js（HLS）播放。回放：先查设备存储文件列表（`/v3/video/files/list`）→ 勾选文件 → `POST /v3/video/playback/start`。演示环境中流媒体后端可能对部分设备返回 1001（内部错误），UI 已做优雅降级提示。
